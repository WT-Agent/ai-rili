  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">查询事项</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">注重维度</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">黄历择吉与时光节点解读指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 黄历宜忌与时光节点解读专家');
...
const metricsList = [
  {
    "key": "traditionalAlmanacAccuracy",
    "label": "黄历典籍契合度"
  },
  {
    "key": "practicalDailyGuidance",
    "label": "日常宜忌实用度"
  },
  {
    "key": "auspiciousTimingPrecision",
    "label": "吉时吉方精准度"
  },
  {
    "key": "modernContextAdaptation",
    "label": "现代场景适配度"
  },
  {
    "key": "culturalHeritageDepth",
    "label": "民俗文化底蕴分"
  }
];
const guideRules = [
  {
    "title": "吉时顺势而为",
    "advice": "重要商业签约或开业剪彩选择辰时、巳时等阳气升发吉时，顺应心理暗示",
    "avoid": "切忌在冲煞凶时进行重大合同盖章签署"
  },
  {
    "title": "理性看待民俗",
    "advice": "将黄历宜忌作为心理正向赋能工具与生活节奏提醒，结合实际天气与安排",
    "avoid": "避免陷入盲目封建迷信而影响正常工作生活"
  },
  {
    "title": "节气顺时养生",
    "advice": "结合节气变化调整饮食作息，春养肝、夏养心、秋养肺、冬养肾",
    "avoid": "严禁违背自然规律强行熬夜或暴饮暴食"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[RILI_SCORES\][\s\S]*?\[\/RILI_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[RILI_SCORES\](.*?)\[\/RILI_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('rili_history_records');
...
const saveHistory = () => {
  localStorage.setItem('rili_history_records', JSON.stringify(historyList.value));
};
